...
- 以资深架构审查标准执行重构：优先保证结构清晰、职责边界明确和长期可维护性，而不是兼容历史垃圾。
- 允许 breaking changes；开发阶段不为旧 API、旧入口、旧数据形态保留兼容层。
- 强制删除重复函数、重复模块、dead code、冗余封装和无意义抽象。
- 强制删除 `v1` / `v2` / `v3` / `old` / `backup` / `temp` / `deprecated` / `legacy` 这类版本堆叠实现与命名。
- 不保留“可能以后用到”的代码；没有当前调用方、明确职责或验证路径的实现必须移除。
- 发现 copy-paste 逻辑时，必须抽取到有领域语义的模块或函数；禁止用 `utils` / `helpers` / `common` 承接杂物。
- 可以修改函数签名、删除 API、重建模块结构；调用方必须迁移到新的清晰边界。
- 每个文件必须有单一职责；一个文件同时承担解析、渲染、存储、传输、业务规则等多种职责时，优先拆分。
- 简化调用链，减少无价值中间层；只保留能表达领域边界、隔离依赖或复用真实逻辑的抽象。
- 重构输出必须说明：发现的问题、删除了什么、合并了什么、为什么这样做，以及核心代码变化。
