...
/**
 * Cross-project ECS substrate browser: filter context-graph nodes by text,
 * project, and substrate layer; inspect a node's detail + relationships.
 *
 * Data is supplied by the host (the ECS dashboard owns the fetch), and the
 * selection is controlled so the host can drive it — e.g. a conflict record
 * deep-linking to one of its nodes. This used to be the standalone
 * `/settings/lineage` page; it now lives inside the ECS page to remove the
 * lineage/project-graph browsing duplication.
 */
export function ContextNodeBrowser({
  nodes,
  edges,
  selectedId,
  onSelect,
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-5">
            ...
            return (
              <div key={s}>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-bold capitalize text-surface-900">{s}</h3>
                ...
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                  {g.map((n) => (
                    ...
                    >
                      ...
                      <h4 className="text-sm font-semibold text-surface-900">{n.title}</h4>
          ...
          {filtered.length === 0 && (
            <div className="rounded-2xl border border-surface-200 p-10 text-center text-sm text-surface-400">
              {t.lineage.noNodes}
            </div>
          )}
        </div>

        <aside className="rounded-2xl border border-surface-200 bg-surface-50 p-5 h-fit">
          <h3 className="text-base font-bold text-surface-900 mb-3">{t.lineage.nodeDetails}</h3>
          {!selected ? (
            ...
            <div className="space-y-4">
              <div>
                ...
                <h4 className="text-base font-bold text-surface-900">{selected.title}</h4>
              ...
              {selected.tags.length > 0 && (
                <div>
                  <h5 className="text-sm font-semibold text-surface-900 mb-2">{t.lineage.tags}</h5>
              ...
              <div>
                <h5 className="text-sm font-semibold text-surface-900 mb-2">{t.lineage.relationships}</h5>
                    ...
                    {selectedEdges.map((e) => {
                      ...
                      return (
                        <button
                          ...
                          className="block w-full text-left rounded-lg border border-surface-100 bg-white p-3 text-sm hover:bg-surface-100"
                        >
