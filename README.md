...
**知识记忆（RAG）**
- 手动 / Git hook / P4 changelist / LLM 四种方式采集知识
- 语义搜索 + 质量评分 + 自动淘汰过期知识
...
- CLI：17 个命令覆盖全部操作
- Repo Scanner：独立的增量仓库扫描工具（第一版支持 `git-local` + 手动 run + daemon）
...
> 也可以使用 `mindstrate setup-mcp` 自动生成配置。
...
> 如果你希望把“定时增量扫描仓库”做成独立工具，而不是混进主记忆逻辑，项目现在提供了一个独立的 `repo-scanner` 原型包。它当前支持 `git-local`、游标持久化、手动 run、daemon、failed item 重试。详见 [packages/repo-scanner/README.md](./packages/repo-scanner/README.md)。
...
# 预览模式
mindstrate capture --recent 5 --dry-run
```
...
p4 triggers
# 添加：
# mindstrate-capture change-commit //depot/... "mindstrate capture --p4 --changelist %changelist% --auto"
```
...
*/30 * * * * mindstrate capture --p4 --recent 5 --auto 2>/dev/null
```

```batch
:: Windows 计划任务
schtasks /create /tn "Mindstrate-P4-Scan" /tr "mindstrate capture --p4 --recent 5 --auto" /sc minute /mo 30
```

**方式三：P4V 自定义工具**

P4V → Tools → Manage Custom Tools → New Tool，Application 填 `node`，Arguments 填 CLI capture 命令。
...
mindstrate capture [options]       从 Git/P4 采集知识
mindstrate hook install|uninstall  管理 git hook
mindstrate export [file]           导出知识库
mindstrate import <file>           导入知识
mindstrate setup-mcp [options]     生成 MCP 配置
mindstrate web [options]           启动 Web UI
mindstrate maintain                质量维护
mindstrate evolve [options]        运行知识进化引擎（支持 --mode background）
mindstrate evaluate [options]      运行检索质量评估
...
│   │   └── src/
│   │       ├── capture/       # 知识采集（Git hook、P4、LLM 提取）
...
│   ├── repo-scanner/          # 独立仓库扫描器（git-local / cursor / daemon / retry）
